...
import { AddFavoriteMealForm } from './AddFavoriteMealForm'
import { FavoriteMealList, FavoriteMealListSkeleton } from './FavoriteMealList'
import { Button } from '@/components/ui/button'

export function FavoriteMealPageClient({ itemId }: { itemId: string }) {
  const [showAddForm, setShowAddForm] = useState(false)

  const item = api.item.getById.useQuery({ id: itemId })
  const meals = api.favoriteMeal.listByItemId.useQuery({ itemId })
    ...
    <div className="flex flex-col h-full">
      <div className="border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-6 py-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div>
            <h1 className="text-xl font-semibold">Favorite Meals</h1>
        ...
        <div className="max-w-2xl mx-auto space-y-4">
          ...
          {showAddForm && (
            <div className="animate-in slide-in-from-top-2 duration-200">
              <AddFavoriteMealForm itemId={itemId} onSuccess={() => { setShowAddForm(false); void meals.refetch() }} />
            </div>
          )}

          {meals.isLoading && <FavoriteMealListSkeleton />}
          {meals.data && <FavoriteMealList itemId={itemId} meals={meals.data} />}
        </div>
      </div>
    </div>
  )
